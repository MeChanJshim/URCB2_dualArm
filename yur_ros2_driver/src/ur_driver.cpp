...
void UrDriver::servoj(std::vector<double> positions, int keepalive) {
	...
	if(bytes_written != 28){
		std::cerr << "Write failed or partial: " << bytes_written;
	}
}
...
bool UrDriver::uploadProg() {
	...
	// --- 목표 조인트 위치 계산 ---
	cmd_str += "\t\t\tq_target = [\n";
	cmd_str += "\t\t\t\tparams_mult[1] / MULT_jointstate,\n";
	...
	// --- servoj 명령 실행 ---
	cmd_str += "\t\t\tservoj(q_target, t=0.008)\n";
...
bool UrDriver::start() {
	...
	int wait_ms = 0;
	while (!rt_interface_->isConnected() && wait_ms < 2000){
		std::this_thread::sleep_for(std::chrono::milliseconds(100));
		wait_ms += 100;
	}

	if(!rt_interface_->isConnected()){
		print_fatal("Failed to connect ro robot for uploading program.");
		return false;
	}

	return UrDriver::uploadProg();
