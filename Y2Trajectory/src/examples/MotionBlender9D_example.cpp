...
int main()
{
    ...
    std::vector<double> velocity = {0.0, 0.5, 0.5, 0.5, 0.5};  // Example velocities (first element is ignored)
    ...
    MotionBlender9D blender(position, velocity, angVelLimit, startingTime, lastRestingTime, accelerationTime, samplingTime);
    ...
    std::string log_dir = source_path.parent_path().string() + "/../log/blended_motion.txt";
